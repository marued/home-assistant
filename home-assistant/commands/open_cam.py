...
class OpenCam(BaseCommand):
    @inject
    def __init__(
        self, command_str: str, type: COMMAND_TYPES, fct: partial = None, close_cam: CloseCam = None
    ) -> None:
        super().__init__(command_str, type, fct)
        file_path = os.path.join(pathlib.Path(__file__).parent.resolve(), "open_cam_config.json")
